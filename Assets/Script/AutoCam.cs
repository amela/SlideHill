...
namespace UnityStandardAssets.Cameras
{
    ...
    public class AutoCam : PivotBasedCameraRig
    {
        ...
        [SerializeField] private float heightY = 10.0f;
        [SerializeField] private float startY = 23.21f;
        [SerializeField] private float startZ = -7.4f;
        [SerializeField] private float endY = -57.0f;
        [SerializeField] private float endZ = 447.2f;
        ...
        protected override void FollowTarget(float deltaTime)
        {
            ...
            //if (m_Target.position.y < targetY)
            //    target = new Vector3(m_Target.position.x, targetY, m_Target.position.z);
            float ratio = (m_Target.position.z - startZ) / (endZ - startZ);
            float targetY = startY + ratio * (endY - startY) + heightY;
            Vector3 target = new Vector3(m_Target.position.x, targetY, m_Target.position.z);
            transform.position = Vector3.Lerp(transform.position, target, deltaTime*m_MoveSpeed);
